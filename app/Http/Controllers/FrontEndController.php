...
class FrontEndController extends Controller
{
    ...
    public function restorant($enterprise, $alias){
        $subDomain=$this->getSubDomain();
        $enterprise = Enterprises::where('name', $enterprise)->first();
        $restorant = Restorant::where('subdomain',$alias)->first();

        if (isset($enterprise) && $restorant->enterprise_id == $enterprise->id) {
            if($subDomain&&$alias!==$subDomain){
                return redirect()->route('restorant',$subDomain);
            }
            // $restorant = Restorant::where('subdomain',$alias)->first();

            if ($restorant->enterprise_id != $enterprise->id) {
                abort(404);
            }
