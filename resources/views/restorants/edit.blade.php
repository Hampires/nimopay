...
@section('admin_title')
    {{ucwords($enterprise->name) . __(' Management')}}
@endsection
@section('content')
                ...
                <div class="card bg-secondary shadow">
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0">{{ ucwords($enterprise->name) . __(' Management') }}</h3>
                                @if (env('WILDCARD_DOMAIN_READY',false))
                                    <span class="blockquote-footer">{{ (isset($_SERVER['HTTPS'])&&$_SERVER["HTTPS"] ?"https://":"http://").$restorant->subdomain.".".str_replace("www.","",$_SERVER['HTTP_HOST']) }}</span>
                                @endif
                            </div>
                            <div class="col-4 text-right">
                                ...
                                @else
                                    <a target="_blank" href="{{ route('vendor',[$enterprise->name, $restorant->subdomain]) }}" class="btn btn-sm btn-success">{{ __('View it') }}</a>
                                @endif
                    ...
                    <div class="card-body">
                       <h6 class="heading-small text-muted mb-4">{{ ucwords($enterprise->name) . __(' information') }}</h6>
