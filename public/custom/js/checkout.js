...
function pricePlan(id, type) {
    ...
    $.ajax({
        ...
        success:function(response) {
            let price = document.querySelector('#plan_' + id + ' .price_amount');
            price.innerHTML = price.innerHTML.substring(0, 1) + response;
        }, error: function (response) {
            //return callback(false, response.responseJSON.errMsg);
        }
    })
...
function makePayment(e, form, data) {
    e.preventDefault();

    FlutterwaveCheckout({
      public_key: "FLWPUBK_TEST-8ed8b2eeeae7a10416ea264a293d6855-X",
      tx_ref: "RX111111111",
      ...
      callback: function (data) { // specified callback function
        ...
        trans_id.name = "transactionId";
        trans_id.value = data.transaction_id;
